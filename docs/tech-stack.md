...
| SPIR-V | offline | Shaders compiled with glslc to .spv at build time |
...
| glslc (Google) | latest | Compile GLSL shaders to SPIR-V |
| Vulkan SDK | latest | Validation layers, glslc, VMA headers |
...
| `tamga_vk3d` | Vulkan, SDL3 (surface), VMA | `tamga_vk3d.zig` |
...
| Shaders | Offline SPIR-V | Runtime GLSL via shaderc | shaderc is large C++; offline has zero runtime cost |
